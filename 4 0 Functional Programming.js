...
filteredBooks = books.filter((book) => book.sales > 1000000);
// console.log(filteredBooks);
  
greatAuthors = filteredBooks.map((book) => {return `${[book.author]} adalah penulis buku ${[book.title]} yang sangat hebat!`});
console.log(greatAuthors);
/* greatAuthors = book.map(
             (books.filter((book) => sales > 100000)) =>
              {return `${book.author} adalah penulis buku ${book.title} yang sangat hebat!`}
              );
  /* /home/glot/main.js:31
              (books.filter((book) => book.sales > 100000))    		                                            ^
  SyntaxError: missing ) after argument list
  /home/glot/main.js:35
